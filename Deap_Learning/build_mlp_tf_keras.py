...
# splite dateset for train and test
...
ax.set_title('Lose duaring training', size=15)
ax.set_xlabel('epochs', size=15)
ax.tick_params(axis='both', which='major', labelsize=15)
ax = fig.add_subplot(1,2,2)
ax.plot(hist['accuracy'], lw=3)
ax.set_title('correctness in teaching', size=15)
ax.set_xlabel('epochs', size=15)
ax.tick_params(axis='both', which='major', labelsize=15)
plt.show()

# estimate model on testing data
results = iris_model.evaluate(ds_test.batch(50), verbose=0)
print(' Lose in testing:  {:.4f}  Right in testing: {:.4f}'.format(*results))


# Save and repeat load trained model
